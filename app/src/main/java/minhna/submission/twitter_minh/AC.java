package minhna.submission.twitter_minh;

/**
 * Created by Administrator on 28-Mar-16.
 */
public class AC {
    public static String CONSUMER_KEY = "tDbfayAz4hNrlGpN107mzSHBT";
    public static String CONSUMER_SECRET = "EKHkan8qLAfuhmdyHfHew7VoBrfOHi3eH8KBrrlGkEa61Mq9OP";
    public static long OWNER_ID = 228622674;

    public static final String BASE_URL = "https://api.twitter.com/1.1";
    public static final String HOME_URL = "/statuses/home_timeline.json";
    public static final String POST_URL = "direct_messages/new.json";
}
